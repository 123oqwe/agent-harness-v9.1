...
phase_reqs = [r for r in reqs if r.get('delivery_phase') == 0]
if not phase_reqs:
    print('FAIL: Phase 0 has no requirements')
    sys.exit(1)
...
if unverified:
    ...
    sys.exit(1)
print(f'PASS: All {len(phase_reqs)} Phase 0 requirements verified')
"

# 3. Phase-specific checks
echo "Phase 0 gate checks complete."

...
phase_reqs = [r for r in reqs if r.get('delivery_phase') == 4]
if not phase_reqs:
    print('FAIL: Phase 4 has no requirements')
    sys.exit(1)
...
if unverified:
    ...
    sys.exit(1)
print(f'PASS: All {len(phase_reqs)} Phase 4 requirements verified')
"

# 3. Phase-specific checks
echo "Phase 4 gate checks complete."

...
phase_reqs = [r for r in reqs if r.get('delivery_phase') == 6]
if not phase_reqs:
    print('FAIL: Phase 6 has no requirements')
    sys.exit(1)
...
if unverified:
    ...
    sys.exit(1)
print(f'PASS: All {len(phase_reqs)} Phase 6 requirements verified')
"

# 3. Phase-specific checks
echo "Phase 6 gate checks complete."

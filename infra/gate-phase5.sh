...
phase_reqs = [r for r in reqs if r.get('delivery_phase') == 5]
if not phase_reqs:
    print('FAIL: Phase 5 has no requirements')
    sys.exit(1)
...
if unverified:
    ...
    sys.exit(1)
print(f'PASS: All {len(phase_reqs)} Phase 5 requirements verified')
"

# 3. Phase-specific checks
echo "Phase 5 gate checks complete."

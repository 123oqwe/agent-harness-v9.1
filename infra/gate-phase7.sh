...
phase_reqs = [r for r in reqs if r.get('delivery_phase') == 7]
if not phase_reqs:
    print('FAIL: Phase 7 has no requirements')
    sys.exit(1)
...
if unverified:
    ...
    sys.exit(1)
print(f'PASS: All {len(phase_reqs)} Phase 7 requirements verified')
"

# 3. Phase-specific checks
echo "Phase 7 gate checks complete."
echo "=== Gate: Phase 7 PASSED ==="

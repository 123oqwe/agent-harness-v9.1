...
phase_reqs = [r for r in reqs if r.get('delivery_phase') == 8]
if not phase_reqs:
    print('FAIL: Phase 8 has no requirements')
    sys.exit(1)
...
if unverified:
    ...
    sys.exit(1)
print(f'PASS: All {len(phase_reqs)} Phase 8 requirements verified')
"

# 3. Phase-specific checks
echo "Phase 8 gate checks complete."
echo "=== Gate: Phase 8 PASSED ==="

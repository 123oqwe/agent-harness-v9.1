...
export async function runResearchVertical(harness: Harness, input: ResearchVerticalInput): Promise<ResearchVerticalOutput> {
  ...
  const observations = outcome.loop_result.turns
    .flatMap((turn) => turn.tool_observations)
    .filter(
      (observation) =>
        observation.name === 'read_file' && observation.status === 'ok',
    );
  const evidence: { source: string; excerpt: string }[] = [];
  for (const src of input.sources) {
    const readResult = observations.find(
      (observation) =>
        (observation.result as { path?: unknown } | undefined)?.path === src,
    );
    if (readResult) {
      const content =
        (readResult.result as { content?: string } | undefined)?.content ?? '';
  ...
  const conflicts = (outcome.verification_report?.records ?? [])
    .flatMap((record) => {
      const value = record.evidence.conflicts;

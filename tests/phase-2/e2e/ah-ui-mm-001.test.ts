...
describe('AH-UI-MM-001: Multimodal Studio screen e2e', () => {
  const screen = PHASE2_SCREENS.find(s => s.id === 'mm');

  it('screen is registered with correct route', () => {
    expect(screen).toBeDefined();
    expect(screen!.path).toBe('/multimodal');
    expect(screen!.title).toBe('Multimodal Studio');
  });
  ...
  it('passes accessibility check with proper attributes', () => {
    const issues = checkAccessibility({ role: 'main', aria_label: 'Multimodal Studio screen', tabindex: 0 });

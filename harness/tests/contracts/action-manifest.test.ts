...
const schemaPath = path.resolve(__dirname, '../../../spec/contracts/action-manifest.schema.json');
const validFixturePath = path.resolve(__dirname, '../../../spec/fixtures/phase-0/valid/action-manifest.json');
const invalidFixturePath = path.resolve(__dirname, '../../../spec/fixtures/phase-0/invalid/action-manifest.json');
...
describe('AH-CONTRACT-ACTIONMANIFEST-001: action-manifest schema', () => {
  ...
  it('schema has required fields: task_id, tool_name, manifest_hash...', () => {
    const schema = JSON.parse(fs.readFileSync(schemaPath, 'utf-8'));
    const required = ["task_id", "tool_name", "manifest_hash"];

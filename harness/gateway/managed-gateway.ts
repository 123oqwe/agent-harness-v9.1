...
import {
  ModelGateway, FrozenProviderRegistry, ProviderDispatchError, ProviderResolutionError,
  type GatewayProviderRegistration, type GatewayProviderMetadata,
...
export class ManagedGateway {
  ...
  constructor(opts: ManagedGatewayOptions = {}) {
    ...
    // Build FrozenProviderRegistry from all providers that have keys
    ...
    const secretsBroker = this.makeSecretsBroker();
    const egressPolicy = this.makeEgressPolicy();
    const usageMeter = this.makeUsageMeter();

    this.modelGateway = new ModelGateway(this.frozenRegistry, { secretsBroker, egressPolicy, usageMeter });
  }
  ...
  private buildMetadata(binding: ModelBinding): GatewayProviderMetadata {
    return {
      ...
      network: {
        required: true,
        destination: binding.api_base,
      },
      credentials: {
        required: true,
        audience: binding.api_base,
      },
    };
  }
  ...
  async complete(prompt: string, ctx: CallContext): Promise<RouteResult> {
    ...
    // 2. Build messages from prompt
    const messages: Message[] = [
      { role: 'system', content: ctx.systemPrompt ?? 'You are a precise agent execution engine. Return ONLY valid JSON.' },
      { role: 'user', content: prompt },
    ];

    // 3. Build ProviderSelectionRequest
    ...
    // 4. Resolve via ModelGateway (hash-verified selection with policy/capability/data-policy checks)
    let resolved;
    try {
    ...
    // 5. Failover loop using ModelGateway.dispatch() + switchProvider()
    ...
    while (true) {
      ...
      if (breaker && !breaker.canRequest()) {
        ...
        try {
          ...
          this.rateLimiter.release(ctx.userId);
          return this.fail('All providers unavailable (circuit breakers open)', ctx, fallbackChain);
        }
      }
      ...
      // Dispatch through the full ModelGateway chain:
      // egress policy → credential exchange → adapter.resolve() → parseResponse() → meterUsage() → usageMeter.record()
      ...
      try {
        ...
        this.keyVault.markHealthy(binding.provider, this.keyVault.getKey(binding.provider) ?? '');
        ...
        // Charge economic kernel with real cost
        if (cost > 0) {
        ...
        // Update the last usage log entry with real cost and latency
        ...
        return {
          ...
          model_used: binding.model_id,
          provider_used: binding.provider,
          fallback_triggered: fallbackChain.length > 1,
          fallback_chain: fallbackChain,
        };
      } catch (e) {
        // Dispatch failed — record on circuit breaker for provider failures only
        if (e instanceof ProviderDispatchError) {
          if (e.code === 'provider_failure' || e.code === 'provider_unhealthy') {
  ...
  private fail(error: string, ctx: CallContext, chain: string[] = [], model = 'none', provider = 'none'): RouteResult {
    return {
      ...
      usage: {
        ...
        timestamp: Date.now(), success: false, error,
      },
